...
class AnomalyMapGenerator(nn.Module):
    """
    Inference : Generate anomaly heatmap

    we use the Mahalanobis distance
    [31] M(xij ) to give an anomaly score to the patch in position
    ...
    N (µij , Σij ), where M(xij )

    Hence, the matrix of Mahalanobis distances
    M = (M(xij ))1<i<W,1<j<H that forms an anomaly
    map can be computed. High scores in this map indicate the
    anomalous areas. The final anomaly score of the entire image
    ...
    def up_sample(self, distance:Tensor):
    ...
    def smooth_anomaly_map(self, anomaly_map: Tensor):
    ...
    def compute_anomaly_map(self, embedding: Tensor, mean: Tensor, inv_covariance: Tensor):
