...
class MultiVariateGaussian(nn.Module):
    ...
    we first compute the set of patch embeddings at that position
    Xij = { xij^k € [1,N]} from the N normal training images.
    To sum up information carried by this set we make the assumption
    that Xij generated by mutlivariate Gaussian distribution.

    So each possible patch position (i,j) is associated with a Gaussian parameters
    mean and inv covariance. 

    we use a multivariate Gaussian distribution to model the distribution of the features.
    """
